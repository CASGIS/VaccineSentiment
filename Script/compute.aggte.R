...
aggte <- function(MP,
  ...
  keepers <- which(t >= group & t >= (group + min_e) & t <= (group + max_e)) ### added second condition to allow for limit on longest period included in att
  ...
  if (type == "group") {
    ...
    selective.att.g <- sapply(glist, function(g) {
      # look at post-treatment periods for group g
      whichg <- which( (group == g) & (group <= t) & ((group + min_e) <= t) & (t <= (group + max_e))) ### added last condition to allow for limit on longest period included in att
      attg <- att[whichg]
      mean(attg)
    })
